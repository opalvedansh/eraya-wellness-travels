...
export default function ToursMap() {
            ...
            try {
                ...
                const processed = data.map((tour: any) => {
                    ...
                    return {
                        ...tour,
                        coordinates: coords,
                        image: tour.coverImage || tour.images?.[0] || "https://images.unsplash.com/photo-1544735716-392fe2489ffa?w=800&fit=crop",
                        rating: tour.rating || 4.5,
                        price: tour.price || 0,
                        duration: tour.duration || "N/A",
                        difficulty: tour.difficulty || 1,
                    };
                });
    ...
    }, [tours, searchQuery, selectedDifficulty]);
            ...
            <div className="flex-1 flex relative overflow-hidden pt-20">
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    <div className="flex items-center justify-between px-2">
                                        <p className="text-sm font-bold text-text-dark/70 flex items-center gap-2">
                                            ...
                                            {filteredTours.length} adventures await
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        {filteredTours.map((tour, index) => (
                                            <motion.button
                                                ...
                                                className={`w-full text-left relative overflow-hidden rounded-xl transition-all duration-300 p-5 space-y-3 ${selectedTour === tour.id ? "border-2 border-green-primary shadow-lg" : "border border-border shadow-sm"}`}
                                            >
                                                <div className="flex items-center justify-between">
                                                    <h3 className="font-black text-base text-text-dark truncate">{tour.name}</h3>
                                                    <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded-lg">
                                                        <span className="text-yellow-600">★</span>
                                                        <span className="font-bold text-yellow-700">{tour.rating}</span>
                                                    </div>
                                                </div>
                                                <p className="text-xs text-text-dark/60 line-clamp-2">{tour.description}</p>
                                                <div className="grid grid-cols-3 gap-2">
                                                    <div className="bg-gray-50 rounded-lg p-2 text-center text-xs font-bold">{tour.duration}</div>
                                                    <div className="bg-green-50 rounded-lg p-2 text-center text-xs font-bold text-green-700">${tour.price}</div>
                                                    <div className="bg-purple-50 rounded-lg p-2 text-center text-xs font-bold text-purple-700">{tour.difficulty}</div>
                                                </div>
                                            </motion.button>
                                        ))}
                                    </div>
                ...
                <div className="flex-1 w-full h-full relative z-0">
                    ...
                    >
                        ...
                        <MapController selectedTourId={selectedTour} tours={tours} />

                        {filteredTours.map((tour) => (
                            <Marker
                                key={tour.id}
                                position={tour.coordinates}
                                icon={selectedTour === tour.id ? activeIcon : defaultMarkerIcon}
                                eventHandlers={{
                                ...
                                <Popup>
                                    <div className="p-2 min-w-[200px]">
                                        ...
                                        <div className="flex gap-2 mb-2">
                                            <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-bold">${tour.price}</span>
                                            <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full font-bold">{tour.duration}</span>
                                        </div>
                                        <button
                ...
                <MapBottomSheet isOpen={bottomSheetOpen} onOpenChange={setBottomSheetOpen}>
                    <div className="p-4">
                        ...
                        <div className="space-y-3">
                            {filteredTours.map(tour => (
                                <div key={tour.id} onClick={() => handleTourClick(tour)} className={`p-3 border rounded-lg ${selectedTour === tour.id ? 'border-green-primary bg-green-50' : ''}`}>
                                    <div className="font-bold">{tour.name}</div>
                                    <div className="text-sm text-gray-600">${tour.price} • {tour.duration}</div>
                                </div>
                            ))}
                        </div>
                    </div>
