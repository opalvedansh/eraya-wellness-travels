...
export default function TrekForm() {
    ...
    const [formData, setFormData] = useState<TrekFormData>(emptyForm);
    ...
    const fetchTrek = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Ensure arrays exist if they are null in DB (e.g. from older records)
                setFormData({
                    ...emptyForm,
                    ...data,
                    ...
                    maxGroupSize: data.maxGroupSize || 12,
                    minAge: data.minAge || 12,
                    coverImage: data.coverImage || "",
                    trekGrade: data.trekGrade || "",
    ...
    const addToArray = (field: keyof TrekFormData, value: string) => {
        ...
        if (field === "bestSeason") setNewSeason("");
    };

    const removeFromArray = (field: keyof TrekFormData, index: number) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                const bookingLink = `${window.location.origin}/booking/trek/${formData.slug}`;
                const message = `Trek ${isEditMode ? "updated" : "created"} successfully!\n\nBooking Link:\n${bookingLink}\n\nClick OK to copy the link.`;
                ...
                if (confirm(message)) {
                    ...
                    navigator.clipboard.writeText(bookingLink).then(() => {
                        ...
                        alert("Could notcopy link, but you can manually copy it from the URL bar.");
                    });
                }
                router.push("/admin/treks");
            } else {
                const error = await response.json();
                alert(`Failed to save trek: ${error.error || "Unknown error"}`);
            }
        } catch (error) {
        ...
        <AdminLayout>
            <div className="p-6">
                <div className="flex items-center gap-4 mb-6">
                    <button onClick={() => router.push("/admin/treks")} className="p-2 hover:bg-gray-100 rounded-lg transition">
                        <ArrowLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">{isEditMode ? "Edit Trek" : "Add New Trek"}</h1>
                        <p className="text-gray-600 mt-1">{isEditMode ? "Update trek information" : "Create a new trekking adventure"}</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" value={formData.name} onChange={handleChange} onBlur={generateSlug} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-primary focus:border-transparent" placeholder="e.g., Everest Base Camp Trek" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">URL Slug *</label>
                                <input type="text" name="slug" value={formData.slug} onChange={handleChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-primary focus:border-transparent" placeholder="everest-base-camp-trek" />
                            </div>
                            <div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="altitude" value={formData.altitude} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-primary focus:border-transparent" placeholder="e.g. 5545m" />
                            </div>
                            <div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Trek Grade</label>
                                <input type="text" name="trekGrade" value={formData.trekGrade} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-primary focus:border-transparent" />
                            </div>
                            <div>
                                    ...
                                    <div className="pl-13 ml-13 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                            <input
                                                type="text"
                                                value={day.altitude || ''}
                                                onChange={(e) => updateItineraryItem(index, 'altitude', e.target.value)}
                                                placeholder="Altitude (e.g. 1400m)"
                    ...
                    {/* Best Season */}
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-4">Best Season</h2>
                        <div className="flex gap-2 mb-3">
                            <input
                                ...
                                onKeyPress={(e) => {
                                    if (e.key === "Enter") {
                                        e.preventDefault();
                                        addToArray("bestSeason", newSeason);
                                    }
                                }}
                            />
                            <button type="button" onClick={() => addToArray("bestSeason", newSeason)} className="px-4 py-2 bg-green-primary text-white rounded-lg hover:bg-green-secondary transition">Add</button>
                        </div>
                        {formData.bestSeason.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {formData.bestSeason.map((season, index) => (
                                    <span key={index} className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-700 rounded-lg text-sm">
                                        {season}
                                        <button type="button" onClick={() => removeFromArray("bestSeason", index)} className="text-blue-900 hover:text-red-600">×</button>
                                    </span>
                                ))}
                            </div>
                        )}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <ImageUpload
                                    ...
                                    helpText="Main image for the trek"
                                />
                    ...
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <p className="text-sm text-gray-600 mb-4">Optimize how this trek appears in search engines</p>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Meta Title
                                ...
                                <input
                                    ...
                                    maxLength={70}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-primary focus:border-transparent"
                                    placeholder="e.g., Everest Base Camp Trek | Eraya Wellness Travels"
                                />
                        ...
                        <div className="space-y-3">
                            <label className="flex items-center gap-3 cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Trek is visible to visitors</p>
                                </div>
                            </label>
                            <label className="flex items-center gap-3 cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Show in featured treks section</p>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <button type="submit" disabled={saving} className="flex items-center px-6 py-3 bg-green-primary text-white rounded-lg hover:bg-green-secondary transition disabled:opacity-50">
                            ...
                            {saving ? "Saving..." : isEditMode ? "Update Trek" : "Create Trek"}
                        </button>
                        <button type="button" onClick={() => router.push("/admin/treks")} className="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">Cancel</button>
                    </div>
                </form>
            </div>
        </AdminLayout>
