...
export default function PaymentConfirmation() {
  ...
  useEffect(() => {
    // Try to get data from sessionStorage (since we can't pass state in Next.js router)
    try {
      const storedData = sessionStorage.getItem("paymentConfirmationData");
      if (storedData) {
        const parsed = JSON.parse(storedData);
        setBookingData(parsed.bookingData);
        setItemData(parsed.itemData);
        setType(parsed.type);
        // Clear after reading
        sessionStorage.removeItem("paymentConfirmationData");
      }
    } catch (e) {
      console.error("Error reading payment confirmation data", e);
    }
  }, []);
    ...
    return (
      <div className="min-h-screen bg-beige flex flex-col">
        ...
        <div className="flex-grow flex flex-col items-center justify-center px-4 py-12">
          ...
          <p className="text-text-dark/70 mb-8 max-w-md text-center">
            Your booking request has been received. Please check your email for confirmation details.
          </p>
          <button
            onClick={() => router.push("/")}
